terraform {
    backend "s3" {
	bucket= "enis-terraform-for-state-file-0125-df27e394" # Replace with your S3 bucket name
...
resource "aws_s3_bucket_object" "private_key_object" {
  bucket = "enis-terraform-for-state-file-0125-44051915"  # Reference to the actual S3 bucket name
  key    = "${var.ssh_key_name}.pem"  # Name of the file in S3 with a .pem extension
  content = tls_private_key.example_ssh_key.private_key_pem  # Content of the private key
  acl    = "private"  # Set ACL to private so only the owner can access it
  server_side_encryption = "AES256"  # Enable encryption for security
}
...
user_data    =  <<-EOF
             #!/bin/bash
             echo "<h1>Hello, World</h1>" > /home/ec2-user/index.html
             python3 -m http.server 8080 > /home/ec2-user/server.log 2>&1 &
             EOF
